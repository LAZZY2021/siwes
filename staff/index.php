...
if (isset($_POST['submit'])) {
    ...
    $resAdmin = mysqli_query($con, $queryAdmin);

    if ($resAdmin && mysqli_num_rows($resAdmin) > 0) {
        $row = mysqli_fetch_assoc($resAdmin);

        // Set admin session variables (kept same as your original)
    ...
    // If not admin, try supervisor table
    $querySuper = "SELECT * FROM supervisor WHERE email='$email' AND password='$password' LIMIT 1";
    $resSuper = mysqli_query($con, $querySuper);

    if ($resSuper && mysqli_num_rows($resSuper) > 0) {
        $row = mysqli_fetch_assoc($resSuper);

        // Set supervisor session variables (kept same as your original)
        $_SESSION['sms_category'] = $row['firstname'];
...
font-size: 27px;"> Siwes Management System</div>
        ...
        <!-- /. NAV SIDE  -->
            ...
            <div class="box-header with-border">
              <h3 class="box-title text-center" style="color:#d0734; font-family:Buxton Sketch; font-size:30px;">Staff Login Portal</h3>
            </div><br>
            <!-- /.box-header -->
            <!-- form start -->
...
<input type="email" class="form-control" id="exampleInputEmail1" name="email" placeholder="Enter Email" required>
                ...
                <!-- Removed dropdown; single email+password form only -->
